...
class EmbeddedPlayer(Gtk.Box):
    def __init__(self):
        ...
        # Create overlay for crop drawing
        ...
        rotation_controls = Gtk.Box(spacing=6)
        ...
        for w in (self.rotate_left_btn, self.rotate_right_btn,
            ...
            rotation_controls.pack_start(w, False, False, 0)

        controls_container.pack_start(main_controls, True, True, 0)
        controls_container.pack_start(rotation_controls, False, False, 0)
    ...
    def apply_video_filters(self):
        ...
        if not getattr(self, "videoflip", None):
            return
...
class NamaCut(Gtk.Window):
    def __init__(self):
        super().__init__(title="NamaCut - Video Cutter v1.0")
        ...
        self.filechooser = Gtk.FileChooserButton(title="Select video", action=Gtk.FileChooserAction.OPEN)
        ...
        for label, tag in formats:
            ...
            if first_btn is None:
                first_btn = btn
                btn.set_active(True)
        vbox.pack_start(fmt_box, False, False, 0)
    ...
    def show_credits(self, widget):
        credits_text = """NamaCut - Video Cutter v1.0

Developer: Pourdaryaei
Website: www.pourdaryaei.ir
Email: Pourdaryaei@yandex.com
"""
        dialog = Gtk.MessageDialog(self, 0, Gtk.MessageType.INFO, Gtk.ButtonsType.OK, credits_text)
        dialog.set_title("About NamaCut")
        dialog.run()
        dialog.destroy()

    def file_filter(self):
        f = Gtk.FileFilter()
        f.set_name("Videos & Audio")
        for p in ("*.mp4", "*.mkv", "*.webm", "*.avi", "*.mov", "*.wmv", "*.flv", "*.m4v",
    ...
    def _set_in(self, *args):
        ...
        if pos is not None:
            ...
            if pos > end_time:
                self.show_dialog("In point can't be after Out point!", error=True)
                return
    ...
    def _set_out(self, *args):
        ...
        if pos is not None:
            ...
            if pos < start_time:
                self.show_dialog("Out point can't be before In point!", error=True)
                return
            h, m, s = seconds_to_hms(pos)
            self.end_h.set_value(h); self.end_m.set_value(m); self.end_s.set_value(s)
    ...
    def show_dialog(self, text, error=False):
        ...
        t = Gtk.MessageType.ERROR if error else Gtk.MessageType.INFO
        d = Gtk.MessageDialog(self, 0, t, Gtk.ButtonsType.OK, text)
        d.run(); d.destroy()
    ...
    def run_ffmpeg_with_progress(self, cmd, total_duration, output_file):
        ...
        try:
            ...
            GLib.idle_add(self.show_dialog, f"Error: {str(e)}", True)
            return False
    ...
    def on_abort(self, widget):
        if self.is_processing and self.current_process:
            ...
            try:
                ...
                pass
            GLib.idle_add(self.status.set_text, "Processing aborted!")
    ...
    def execute_post_action(self, action, output_file):
        try:
            ...
            GLib.idle_add(lambda: self.show_dialog(f"Failed to execute action: {str(e)}", error=True))
    ...
    def on_trim(self, btn):
        ...
        if not path:
            self.show_dialog("No input file selected.", error=True)
            return
        ok, msg = self.validate_times()
        if not ok:
            self.show_dialog(msg, error=True)
            return
        ...
        if fmt is None:
            self.show_dialog("Select one output format.", error=True)
            return
        ...
        def do_trim():
            ...
            try:
                ...
                if fmt == "original":
                    ...
                    # choose codec set per container
                    if fmt == "mp4":
                        ...
                        codec = ["-c:v", "wmv2", "-qscale:v", "3", "-c:a", "wmav2", "-b:a", "192k"]
                    else:
                        codec = ["-c:v", "libx264", "-preset", "medium", "-crf", "23", "-c:a", "aac", "-b:a", "192k"]
                ...
                if success and out:
                    ...
                    if action != "No action":
                        ...
                        GLib.idle_add(self.show_dialog, "Processing completed!")
                else:
                    GLib.idle_add(lambda: self.show_dialog("Failed to export. Check console.", True))
                    GLib.idle_add(self.status.set_text, "Export failed.")

            except Exception as e:
                GLib.idle_add(lambda: self.show_dialog(f"Error: {str(e)}", True))
